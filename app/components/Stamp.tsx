...
export function stampClass(f: Flag): string {
  ...
  if (f.rule === "post.eligibility") return f.status === "yes" ? "stamp stamp-green" : f.status === "no" ? "stamp stamp-red" : f.status === "watch" ? "stamp stamp-amber" : "stamp stamp-ink";
...
export function stampLabel(f: Flag): string | null {
  ...
  if (f.rule === "post.eligibility") return f.status === "yes" ? "OCTOBER OK" : f.status === "watch" ? "OCTOBER: REINSTATE" : f.status === "no" ? "OCTOBER: OUT" : "OCTOBER: SUB ONLY";
